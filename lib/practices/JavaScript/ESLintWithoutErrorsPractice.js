...
let ESLintWithoutErrorsPractice = class ESLintWithoutErrorsPractice extends PracticeBase_1.PracticeBase {
    ...
    async runEslint(ctx, { fix } = { fix: false }) {
        ...
        const securityVulnerabilitiesPracticeDebug = utils_1.debugLog('ESLintWithoutErrorsPractice');
        ...
        if (eslintConfig.length > 0) {
            ...
            try {
                ...
                if (eslintConfig[0].extension === '.yml' || eslintConfig[0].extension === '.yaml') {
                    baseConfig = js_yaml_1.default.load(await ctx.fileInspector.readFile(eslintConfig[0].path));
                }
                else {
                ...
                options = Object.assign(Object.assign({}, options), { baseConfig, overrideConfig: {
                        plugins,
                        ignorePatterns,
                        //make sure that eslint knows path to tsconfig
                        parserOptions: { tsconfigRootDir: ctx.fileInspector.basePath },
                    }, overrideConfigFile: eslintConfig[0].path, resolvePluginsRelativeTo: `${ctx.fileInspector.basePath}/node_modules` });
            }
            catch (error) {
                const eSLintWithoutErrorsPracticeDebug = utils_1.debugLog('ESLintWithoutErrorsPractice');
