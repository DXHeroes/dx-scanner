...
const debug = debug_1.default('cli:services:git:gitlab-service');
let GitLabService = class GitLabService {
    ...
    async listContributors(owner, repo, options) {
        const commits = await this.getAllCommits(`${owner}/${repo}`, options === null || options === void 0 ? void 0 : options.pagination);
        const items = await Promise.all(commits
    ...
    async getAllCommits(projectId, pagination) {
