...
describe('FileSystemService - REAL', () => {
    ...
    beforeAll(async () => {
        const container = inversify_config_1.createTestContainer({ uri: '.' });
        fileSystemService = container.fileSystemService;
    });
    describe('#exists', () => {
        it('returns true if the file exists', async () => {
            const mockFilePath = path_1.default.resolve(__dirname, '__MOCKS__/mockFolder/mockFile.ts');
            const result = await fileSystemService.exists(mockFilePath);
            expect(result).toEqual(true);
        });
        it('returns true if the directory exists', async () => {
            const mockFolderPath = path_1.default.resolve(__dirname, '__MOCKS__/mockFolder');
            const result = await fileSystemService.exists(mockFolderPath);
            expect(result).toEqual(true);
        });
        it("returns false if the file doesn't exists", async () => {
            const mockFolderPath = path_1.default.resolve(__dirname, '__MOCKS__/notExistingMockFolder');
            const result = await fileSystemService.exists(mockFolderPath);
    ...
    describe('#readFile', () => {
        ...
        it("throws an error if the target doesn't exist", async () => {
            const mockFilePath = path_1.default.resolve(__dirname, '__MOCKS__/notExistingMockFolder');
    ...
    describe('#writeFile', () => {
        it('correctly writes to the file', async () => {
            const mockFilePath = path_1.default.resolve(__dirname, '__MOCKS__/notExistingMockFile.ts');
        ...
        it('correctly rewrites a file', async () => {
            const mockFileToRewrite = path_1.default.resolve(__dirname, '__MOCKS__/mockFolder/mockFileToRewrite.ts');
        ...
        it("throws an error if the parent directory doesn't exist", async () => {
            const mockFilePath = path_1.default.resolve(__dirname, '__MOCKS__/notExistingMockFolder/file.ts');
        ...
        it('throws an error if the parent directory is not a directory', async () => {
            const mockFilePath = path_1.default.resolve(__dirname, '__MOCKS__/mockFolder/mockFile.ts/file.ts');
            await expect(fileSystemService.writeFile(mockFilePath, '...')).rejects.toThrow(/ENOTDIR|ENOENT/);
        });
        it("throws an error if the target isn't a file", async () => {
            const mockFilePath = path_1.default.resolve(__dirname, '__MOCKS__/mockFolder');
    ...
    describe('#createFile', () => {
        ...
        it('appends data to a file if it exists already', async () => {
            const mockFile = path_1.default.resolve(__dirname, '__MOCKS__/mockFolder/mockFile.ts');
    ...
    describe('#deleteFile', () => {
        it('returns false after calling exists() when the file is deleted', async () => {
            const filePath = path_1.default.resolve(__dirname, '__MOCKS__/testMockFile.ignore');
    ...
    describe('#createDirectory', () => {
        it('exists() returns true after creating directory', async () => {
            const mockFolderToDelete = path_1.default.resolve(__dirname, '__MOCKS__/createDirTest');
            try {
        ...
        it.skip('throws an error if the target is a broken symbolic link', async () => {
            const mockFilePath = path_1.default.resolve(__dirname, '__MOCKS__/mockFolder/mockFileSLbroken.ln');
            await expect(fileSystemService.createDirectory(mockFilePath)).rejects.toThrow('EEXIST');
        });
    });
    describe('#deleteDirectory', () => {
        it('exists() returns false after deleting', async () => {
            const mockFolderToDelete = path_1.default.resolve(__dirname, '__MOCKS__/deleteDirTest');
            await fileSystemService.createDirectory(mockFolderToDelete);
            await fileSystemService.deleteDirectory(mockFolderToDelete);
            const existsDir = await fileSystemService.exists(mockFolderToDelete);
        ...
        describe('#getMetadata', () => {
            ...
            it('it returns metadata for dotfile', async () => {
                const mockFilePath = path_1.default.resolve(__dirname, '__MOCKS__/.keep');
                const result = await fileSystemService.getMetadata(mockFilePath);
            ...
            it.skip('it returns metadata for broken Symlink but only as a File', async () => {
                const mockFilePathSL = path_1.default.resolve(__dirname, '__MOCKS__/mockFolder/mockFileSLbroken.ln');
        ...
        describe('#flatTraverse', () => {
            ...
            it("throws an error if the root isn't a directory", async () => {
                const mockFolderPath = path_1.default.resolve(__dirname, '__MOCKS__/mockFolder/mockFile.ts');
                await expect(fileSystemService.flatTraverse(mockFolderPath, () => true)).rejects.toThrow('ENOTDIR');
            });
        });
    });
});
//# sourceMappingURL=FileSystemService.real.spec.js.map