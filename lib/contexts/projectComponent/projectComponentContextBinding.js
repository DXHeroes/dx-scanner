...
const createProjectComponentContainer = (projectComponent, rootContainer) => {
    ...
    container.bind(types_1.Types.PracticeContextFactory).toFactory((ctx) => {
        return (projectComponent) => {
            let gitInspector;
            let issueTrackingInspector;
            let collaborationInspector;
            try {
                gitInspector = ctx.container.get(types_1.Types.IGitInspector);
                issueTrackingInspector = ctx.container.get(types_1.Types.IIssueTrackingInspector);
                collaborationInspector = ctx.container.get(types_1.Types.ICollaborationInspector);
            }
            catch (_a) { }
            return {
